...
function anagrams(stringA, stringB) {
  ...
  let cleanedStrA = stringA.replace(/[^\w]/g, "").toLowerCase();
  let cleanedStrB = stringB.replace(/[^\w]/g, "").toLowerCase();

  if (cleanedStrA.length !== cleanedStrB.length) {
    return false;
  }

  let sortedStrA = cleanedStrA
    .split("")
    .sort((a, b) => a > b)
    .join("");
  let sortedStrB = cleanedStrB
    .split("")
    .sort((a, b) => a > b)
    .join("");
